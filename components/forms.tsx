...
// Add title
// Space after logo
page.drawText('Asistencia Médica - Reporte Médico', {
  x: margin,
  y: currentY,
  size: 24,
  font: helveticaFont,
...
// Add title
page2.drawText('Asistencia Médica - Reporte Médico', {
  x: margin,
  y: currentY2,
  size: 24,
  font: helveticaFont,
...
// Add title
page3.drawText('Asistencia Médica  - Reporte Médico', {
  x: margin3,
  y: currentY3,
  size: 24,
  font: helveticaFont,
 ...
 const maxScores = glasgowData.map(row => Math.max(...row.scores.map(s => s.score)));
  ...
  return (
    <motion.div className="p-4 max-w-3xl pb-6 mx-auto bg-white shadow-md rounded-2xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
      <h1 className="text-2xl font-bold mb-4">Formulario de Paciente - Asistencia Médica </h1>
